...
## B.6 高级特性与测试（ch22–ch26）

- [ ] 多模态链路六段复述：抓取（SSRF 白名单）→ 解码 → NIXL 直达显存 →
      Encode worker → prefill → 路由记账。
- [ ] 多模态路由三机制：占位符哈希归一化、`to_block_level` 块级记账、
      嵌入缓存。
- [ ] 四个多模态部署警告（前端 GPU/镜像/VP8-VP9 白名单/限制不可运行时改）。
- [ ] LoRA 为什么必须进块哈希而不是路由层软过滤？引擎侧的实现路径？
- [ ] `LoRADownloader` 三种 URI 的行为差异；适配器驻留准入子系统五组件。
- [ ] "worker 挂了"五问拆解；游标状态机四态（Initial/Contiguous/Gap/Stale）
      各自的处置。
- [ ] 迁移排除链：`excluded_worker_ids` → 重路由 → `exhausted_error`；
      `free_if_worker` 防什么。
- [ ] 共享缓存价值阶梯（1.0/0.75/0.5/0.25/0）与 hicache 客户端三细节。
- [ ] 跨 DC 的"同步成员、按需搬块"原则；哈希域一致性问题。
- [ ] `run_serve_deployment` 四步流水；日志契约为什么是路由回归的载体。
